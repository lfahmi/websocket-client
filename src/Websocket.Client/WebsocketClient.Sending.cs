...
namespace Websocket.Client
{
    public partial class WebsocketClient
    {
        ...
        public void Send(byte[] message, int offset, int length)
        {
            ...
            _messagesBinaryToSendQueue.Writer.TryWrite(
                new ArraySegment<byte>(message, offset, length > 0 ? length : message.Length - offset));
        }
        ...
        {
            return SendInstant(message, 0, message.Length);
        }

        public Task SendInstant(byte[] message, int offset = 0, int length = 0)
        {
            return SendInternalSynchronized(new ArraySegment<byte>(message, offset, length > 0 ? length : message.Length - offset));
        }
